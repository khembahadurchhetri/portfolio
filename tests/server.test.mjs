...
test('owner authentication, CSRF, persistent CRUD and private media isolation', async () => {
  ...
  try {
    ...
    const portfolio = await (await request('/api/portfolio', 'GET', undefined, false)).json();
    assert.ok(portfolio.projects.every(project => project.codeUrl));
    assert.ok(!portfolio.projects.some(project => project.title === 'Interface Design Studies'));
    assert.equal((await request('/api/admin/portfolio', 'GET', undefined, false)).status, 401);
    assert.equal((await request('/api/admin/portfolio', 'PUT', portfolio, false)).status, 401);
    assert.equal((await request('/api/admin/portfolio', 'PUT', portfolio, true, {'X-CSRF-Token':'wrong'})).status, 403);
    assert.equal((await request('/api/admin/portfolio', 'PUT', {...portfolio, cv:'javascript:alert(1)'})).status, 400);
    assert.equal((await request('/api/admin/portfolio-media', 'POST', Buffer.from('%PDF-1.7\nTest'), false)).status, 401);
    assert.equal((await request('/api/admin/portfolio-media', 'POST', Buffer.from('<svg/>'))).status, 415);
    const cvResponse = await request('/api/admin/portfolio-media', 'POST', Buffer.from('%PDF-1.7\nTest'));
    assert.equal(cvResponse.status, 201);
    portfolio.cv = (await cvResponse.json()).url;
    portfolio.projects[0].title = 'Updated project';
    assert.equal((await request('/api/admin/portfolio', 'PUT', portfolio)).status, 200);
    assert.equal((await (await request('/api/portfolio', 'GET', undefined, false)).json()).projects[0].title, 'Updated project');
    assert.equal((await request(portfolio.cv, 'GET', undefined, false)).headers.get('content-type'), 'application/pdf');
    ...
    assert.equal((await (await request('/api/portfolio', 'GET', undefined, false)).json()).cv, portfolio.cv);
