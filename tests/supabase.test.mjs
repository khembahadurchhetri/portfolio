...
test('Supabase adapter separates public reads, owner auth and uploads',async()=>{
  ...
  vm.runInNewContext(await readFile('assets/journal-backend.js','utf8'),{window,Blob,Date,AbortSignal,encodeURIComponent,crypto:{randomUUID:()=> '11111111-1111-4111-8111-111111111111'},fetch:async(url,options)=>{
    ...
    else if(url.includes('portfolio_content') && options.method === 'POST')data=[JSON.parse(options.body)];
    else if(options.headers.Prefer)data=[{id:'saved',visibility:'private'}];
    return {ok:true,status:200,json:async()=>data};
  }});
  const backend=window.JournalBackend;
  await assert.rejects(backend.request('/api/admin/portfolio', {method:'PUT', body:'{}'}), {status:401});
  await assert.rejects(backend.request('/api/admin/portfolio-media', {method:'POST', body:new Blob(['pdf'], {type:'application/pdf'})}), {status:401});
  await backend.request('/api/portfolio');
  ...
  await backend.request('/api/admin/portfolio', {method:'PUT', body:JSON.stringify({photo:'/assets/photo.jpg',cv:'/assets/cv.pdf',projects:[]})});
  assert.match(calls.at(-1).url,/portfolio_content\?on_conflict=id/);
  assert.equal(calls.at(-1).headers.Authorization,'Bearer owner-token');
  assert.equal(JSON.parse(calls.at(-1).body).id,1);
  const cv = await backend.request('/api/admin/portfolio-media',{method:'POST',body:new Blob(['%PDF-1.7'],{type:'application/pdf'})});
  assert.match(cv.url,/\/object\/public\/portfolio-assets\/.+\.pdf$/);
  assert.equal(calls.at(-1).headers.Authorization,'Bearer owner-token');
  await assert.rejects(backend.request('/api/admin/portfolio-media',{method:'POST',body:new Blob(['svg'],{type:'image/svg+xml'})}), /Choose a/);
